...
def ar1_gaussian(n, phi=0.8, sigma_eps=1.0, burn=2000):
    """
    X_t = phi X_{t-1} + eps_t,  eps_t ~ N(0, sigma_eps^2)
    |phi|<1 -> strikt stationär, geometrisch alpha-mixing.
    Simulation startet aus stationärer Randverteilung + burn-in.
    """
    if not (-1.0 < phi < 1.0):
...
def mc_Z_ar1(n, eps, R, phi=0.8, sigma_eps=1.0, progress=1000):
    # Randverteilung von AR(1) ist normal
    sigma_stat = sigma_eps / np.sqrt(1.0 - phi**2)
